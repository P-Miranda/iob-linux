/*
 * vinput.c
 * 1. insert kernel module: insmod vinput.ko
 * 2. create vinputX sysfs entry: echo "vkbd" > /sys/class/vinput/export
 * 3. manually create char device: sudo mknod /dev/vinput0 c [major] 0
 *     check vinput0 major with: cat /proc/devices | grep vinput
 * 4. unexport device: echo "0" > /sys/class/vinput/unexport
 */
...
static int vinput_release(struct inode *inode, struct file *file) { return 0; }
