/*
 * completions.c - force a specific sequence of execution between two threads
 * 1. insert module: sudo insmod completions.ko
 * 2. check thread order: dmesg
 */
...
static int __init completions_init(void) {
  ...
  flywheel_thread =
      kthread_create(machine_flywheel_spinup_thread, NULL, "KThread Flywheel");
